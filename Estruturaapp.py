...
st.write("Suba a foto do caderno. O sistema tentará mapear a geometria, e você poderá ajustar os nós e barras livremente.")
...
if arquivo_imagem is not None:
    ...
    with col1:
        ...
        st.info(f"O OpenCV sugeriu {num_nos_sugeridos} nós.")

    with col2:
        st.subheader("⚙️ Validação e Ajuste de Nós/Barras")
        
        num_nos_reais = st.number_input(
            "Quantidade REAL de nós da estrutura:", 
        ...
        no_carga = st.selectbox("Selecione o Nó da Força Concentrada:", [f"Nó {i+1}" for i in range(num_nos_reais)], index=1)
        ...
        val_q = 0.0
        if tem_distribuida:
            val_q = st.number_input("Intensidade da Carga Distribuída (kN/m):", value=-10.0)
    ...
    if st.button("🚀 Resolver Estrutura via Rigidez Direta"):
        try:
            ...
            if num_nos_reais == 4:
                h1, L_viga, h2 = comprimentos[0], comprimentos[1], comprimentos[2]
                coords = {1: [0.0, 0.0], 2: [0.0, h1], 3: [L_viga, h1], 4: [L_viga, h1 - h2]}
    ...
    st.warning("Insira a foto para iniciar.")
